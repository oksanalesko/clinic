(() => {
    ...
    function initSliders() {
        if (document.querySelector(".hero__slider")) {
            ...
            document.addEventListener("click", (function(e) {
                const targetElement = e.target;
                if (targetElement.classList.contains("swiper-pagination-bullet") && targetElement.closest(".hero__pagination")) {
                    const sliderId = +targetElement.dataset.slide;
                    heroSlider.slideTo(sliderId, 800);
                }
            }));
        }
        if (document.querySelector(".related-news__slider")) {
            const heroSlider = new swiper_core_Swiper(".related-news__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
                ...
                loop: true,
                navigation: {
                    prevEl: ".related-news__arrow--prev",
                    nextEl: ".related-news__arrow--next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
        ...
        if (document.querySelector(".related-products__slider")) {
            const heroSlider = new swiper_core_Swiper(".related-products__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 4,
    ...
    class ScrollWatcher {
        ...
        getScrollWatcherConfig(paramsWatch) {
            let configWatcher = {};
            if (document.querySelector(paramsWatch.root)) configWatcher.root = document.querySelector(paramsWatch.root); else if (paramsWatch.root !== "null") this.scrollWatcherLogging(`Eмм... батьківського об'єкта ${paramsWatch.root} немає на сторінці`);
...
***************************************************************************** */
    ...
    if (certificates) loadCertificates();
    async function loadCertificates() {
        ...
        if (response.ok) {
            ...
            initCertificates(responseResult);
        } else alert("Error");
    }
    const doctorsList = document.querySelector(".body-certificates__list");
    const doctorsPhoto = document.querySelector(".body-certificates__photo");
    const doctorsSkills = document.querySelector(".info-certificates__skills");
    const doctorsStat = document.querySelector(".statistic-info");
    const doctorsPortfolio = document.querySelector(".portfolio-certificates__list");
    const doctorsListActiveElement = `_active`;
    function initCertificates(data) {
        let doctorsListElements = ``;
        data.doctors.forEach((doctor => {
            const isActive = doctor.isActive;
            if (isActive) buildCertificates(doctor);
            doctorsListElements += `\n         <li class="body-certificates__item">\n\t\t\t   <a data-id="${doctor.id}" href="#" class="body-certificates__card card-certificates ${isActive ? doctorsListActiveElement : null}">\n\t\t\t\t   <div class="card-certificates__avatar-ibg">\n\t\t\t\t\t   <img src="${doctor.media.avatar}" alt="Avatar">\n\t\t\t\t   </div>\n\t\t\t\t   <div class="card-certificates__content">\n\t\t\t\t\t   <div class="card-certificates__name">${doctor.name}</div>\n\t\t\t\t\t   <div class="card-certificates__text text text--white">\n\t\t\t\t\t\t   ${doctor.position}\n\t\t\t\t\t   </div>\n\t\t\t\t   </div>\n\t\t\t   </a>\n\t\t   </li>\n      `;
        }));
        doctorsList.innerHTML = doctorsListElements;
        document.addEventListener("click", doctorsListActions);
        function doctorsListActions(e) {
            const targetElement = e.target;
            if (targetElement.closest(".card-certificates")) {
                const currentItem = targetElement.closest(".card-certificates");
                const doctorId = +currentItem.dataset.id;
                const activeListItem = document.querySelector(".card-certificates._active");
                ...
                buildCertificates(activeDoctor);
                e.preventDefault();
            }
        }
    }
    function buildCertificates(activeDoctor) {
        let skillItem = ``;
        let statItem = ``;
        let portfolioItem = ``;
        ...
        const portfoliosSmall = activeDoctor.media.gallery.small;
        const portfoliosBig = activeDoctor.media.gallery.big;
        mainPhoto ? doctorsPhoto.innerHTML = `<img src="${mainPhoto}" alt="Photo">` : null;
        for (const item in skills) skillItem += `\n               <li class="info-certificates__item-skills item-skills">\n\t\t\t\t      <div style="min-width: ${skills[item]}%;" class="item-skills__info">\n\t\t\t\t\t      <div class="item-skills__label">${item}</div>\n\t\t\t\t\t      <div class="item-skills__value">${skills[item]}%</div>\n\t\t\t\t      </div>\n\t\t\t\t      <div style="width: ${skills[item]}%" class="item-skills__line"></div>\n\t\t\t      </li>\n            `;
        doctorsSkills.innerHTML = skillItem;
        for (const item in stats) statItem += `\n            <li class="statistic-info__item">\n\t\t\t\t\t<div data-digits-counter data-digits-counter-speed="3000" class="statistic-info__value">${stats[item]}</div>\n\t\t\t\t\t<div class="statistic-info__label">${item}</div>\n\t\t\t\t</li>\n            `;
        doctorsStat.innerHTML = statItem;
        portfoliosSmall.forEach(((item, index) => {
            portfolioItem += `\n               <a href="${item}" class="portfolio-certificates__item-ibg">\n\t\t\t\t\t   <img src="${portfoliosBig[index]}" alt="Portfolio case">\n\t\t\t\t   </a>\n            `;
        }));
        doctorsPortfolio.innerHTML = portfolioItem;
    ...
    if (gallery && !isMobile.any()) window.addEventListener("scroll", (function(e) {
        ...
        const galleryRows = document.querySelectorAll(".gallery-about__row");
        const windowHeight = window.innerHeight;
        galleryRows.forEach(((galleryRow, index) => {
            const galleryItems = galleryRow.querySelectorAll(".item-gallery");
            let dRowWidth = 0;
            galleryItems.forEach((galleryItem => {
                dRowWidth += galleryItem.offsetWidth + 20;
            }));
            dRowWidth -= 20;
            const galleryWay = dRowWidth - galleryWidth;
            if (galleryWay > 0) {
                const galleryRowHeight = galleryRow.offsetHeight;
                const galleryRowTopPos = galleryRow.getBoundingClientRect().top;
                const galleryRowTop = windowHeight - galleryRowTopPos;
                const slideDirection = index % 2 === 0 ? -1 : 1;
                if (galleryRowTop >= galleryRowHeight) {
                    const sWay = (galleryRowTop - galleryRowHeight) / (windowHeight - galleryRowHeight) * 100;
                    galleryRow.style.cssText = `\n                  transform: translateX(${galleryWay / 100 * sWay * slideDirection}px);\n               `;
                    if (galleryRowTop >= windowHeight) galleryRow.style.cssText = `\n                     transform: translateX(${galleryWay * slideDirection}px);\n                  `;
                } else galleryRow.style.cssText = `\n                  transform: translateX(0px);\n               `;
            }
        }));
    }));
    let stepCards = document.querySelectorAll(".steps-about__card");
    stepCards.forEach((function(stepCard, index) {
        let itemNumber = stepCard.querySelector(".steps-about__number");
        if (itemNumber) itemNumber.innerText = (index + 1).toString();
    }));
    const input = document.querySelector("#range");
    const rangeValue = document.querySelector("#range-value");
    if (input && rangeValue) input.addEventListener("input", (function() {
        rangeValue.textContent = input.value;
    }));
    document.addEventListener("click", (function(e) {
        const targetElement = e.target;
        if (targetElement.closest(".checkbox") && targetElement.closest(".filter-block") && targetElement.tagName !== "INPUT") filterClearShow(targetElement);
        if (targetElement.closest(".filter-block__clear")) {
            const clearButton = targetElement.closest(".filter-block__clear");
            filterBlockClear(clearButton);
        }
    }));
    function filterClearShow(targetElement) {
        const filterBlock = targetElement.closest(".filter-block");
        const filterClear = filterBlock.querySelector(".filter-block__clear");
        setTimeout((() => {
    ...
    function filterBlockClear(clearButton) {
        const filterBlock = clearButton.closest(".filter-block");
        const checkedElements = filterBlock.querySelectorAll(".checkbox__input:checked");
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        let deleteButtons = document.querySelectorAll(".shop-single-product__delete");
        deleteButtons.forEach((function(button) {
            button.addEventListener("click", (function() {
                let item = button.closest(".shop-single-product__item");
                if (item) item.remove();
            }));
        }));
    }));
